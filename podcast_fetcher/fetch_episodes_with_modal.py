...
from podcast_fetcher.analyze_transcripts import generate_episode_summary
from podcast_fetcher.database import (
    init_database,
    is_episode_processed_for_user,
...
def fetch_podcast_episodes():
    ...
    with Session(engine) as session:
        ...
        if not podcasts:
            print(
                "No podcasts found. Add a podcast using 'python -m podcast_fetcher.add_podcast add'"
            )
            return
        ...
        for podcast in podcasts:
            ...
            try:
                ...
                for episode_data in episodes:
                    ...
                    saved_count += 1
                    print(
                        f"Added: {episode.title} ({episode.published.strftime('%Y-%m-%d')})"
                    )
    ...
    with Session(engine) as session:
        ...
        for job in jobs:
            try:
                ...
                print(
                    f"Updated episode {episode.id} with transcript from job {job.job_name}"
                )
...
def process_immediate_notifications_logic(engine, session):
    ...
    for podcast_id, usernames in podcast_subscriptions.items():
        try:
            ...
            # Use 10-day lookback to match episode fetch window
            ...
            for episode in episodes:
                try:
                    ...
                    for username in usernames:
                        if not is_episode_processed_for_user(
                            engine, episode.id, username
                        ):
                            success = process_episode_for_user(
                                engine, episode, username
                            )
                            if success:
                                mark_episode_processed(
                                    engine, episode.id, username, True
                                )
                                total_notifications += 1
                                print(
                                    f"    Sent immediate notification to {username} for: {episode.title}"
                                )
                            else:
                                total_errors += 1
                                print(
                                    f"    Failed to send immediate notification to {username} for: {episode.title}"
                                )
    ...
    print(
        f"Immediate notifications: {total_processed} processed, {total_notifications} sent, {total_errors} errors"
    )
    ...
    for username, episodes in user_episodes.items():
        if episodes:
            ...
            if success:
                ...
                total_sent += 1
                print(
                    f"  Sent daily digest to {username} with {len(episodes)} episodes"
                )
    ...
    for username, episodes in user_episodes.items():
        if episodes:
            ...
            if success:
                ...
                total_sent += 1
                print(
                    f"  Sent weekly digest to {username} with {len(episodes)} episodes"
                )
...
def format_episode_notification(episode: Episode, summary: str) -> str:
    """Format episode notification message using HTML for better reliability.
    ...
    escaped_podcast_title = html.escape(episode.podcast.title if episode.podcast else 'Unknown Podcast')
    escaped_episode_title = html.escape(episode.title)
    escaped_summary = html.escape(summary)
    
    message = f"🎧 <b>New Episode Available!</b>\n\n"
    message += f"📻 <b>{escaped_podcast_title}</b>\n"
    message += f"📺 <b>{escaped_episode_title}</b>\n"
    message += f"📅 {episode.published.strftime('%Y-%m-%d')}\n"
    message += f"🔗 {episode.link}\n\n"
    message += f"<b>Summary:</b>\n{escaped_summary}\n\n"
    message += f"---\n"
    message += f"<i>You're receiving this because you're subscribed to this podcast.</i>"
...
def send_telegram_notification_sync(username: str, message: str) -> bool:
    ...
    try:
        ...
        # Create client with the specific event loop
        ...
        async def send_message_async():
            try:
                ...
                for i, chunk in enumerate(message_chunks, 1):
                    ...
                    await client.send_message(f"@{username}", chunk, parse_mode='html')
                    print(f"Message part {i}{progress} sent to @{username} successfully!")
        ...
        # Execute the async function using run_until_complete
...
def format_daily_digest(username: str, episodes: List[Episode]) -> str:
    """Format daily digest message using HTML.
    ...
    message = f"📰 <b>Daily Podcast Digest</b>\n\n"
    ...
    for podcast_title, podcast_episodes_list in podcast_episodes.items():
        escaped_podcast_title = html.escape(podcast_title)
        message += f"📻 <b>{escaped_podcast_title}</b>\n"
        for episode in podcast_episodes_list:
            escaped_title = html.escape(episode.title)
            message += (
                f"  • {escaped_title} ({episode.published.strftime('%Y-%m-%d')})\n"
            )
            message += f"    🔗 {episode.link}\n"
        message += "\n"

    message += f"---\n"
    message += f"<i>You're receiving this daily digest because you're subscribed to these podcasts.</i>"

    return message


def format_weekly_digest(username: str, episodes: List[Episode]) -> str:
    """Format weekly digest message using HTML.
    ...
    message = f"📰 <b>Weekly Podcast Digest</b>\n\n"
    ...
    for podcast_title, podcast_episodes_list in podcast_episodes.items():
        escaped_podcast_title = html.escape(podcast_title)
        message += f"📻 <b>{escaped_podcast_title}</b> ({len(podcast_episodes_list)} episodes)\n"
        for episode in podcast_episodes_list:
            escaped_title = html.escape(episode.title)
            message += (
                f"  • {escaped_title} ({episode.published.strftime('%Y-%m-%d')})\n"
    ...
    message += f"---\n"
    message += f"<i>You're receiving this weekly digest because you're subscribed to these podcasts.</i>"
